...
`.dcap` 采用 **v2 单文件容器**：消息按 chunk 分组并整体压缩（默认 deflate，约 2.5–2.8x），
文件末尾写 chunk 时间索引；读端可按时间 `seek` 只解压目标 chunk，实现**长录制（数十 GB 原始）下的
有界内存流式回放**。旧版 v1（无压缩线性格式）文件仍可读取，规范消息 `rbag1` wire 布局不变。
